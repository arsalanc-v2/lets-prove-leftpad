...
Since we use recursion, we need to prove that the function eventually terminates. We do this using the `decreases` clause, stating  `n - |s|` since the length of `s` increases in every recursive call while `n` stays the same. Dafny then verifies that this value is bounded in that it cannot decrease infinitely. For simpler expressions such as a single integer, Dafny is able to infer the decreasing expression automatically.
